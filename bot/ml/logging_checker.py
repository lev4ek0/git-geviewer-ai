...
from ml.utils import add_line_numbers
...
class LoggingCheckerOutput(BaseModel):
    ...
    start_line_number: int = Field(
        description="Начало участка кода где есть ошибка (номер строки)"
    )
    end_line_number: int = Field(
        description="Конец участка кода где есть ошибка (номер строки)"
    )
...
class ListLoggingCheckerOutput(BaseModel):
    comments: List[LoggingCheckerOutput] = Field(
        description="Список ошибок найденных в коде"
    )
...
class LoggingChecker:
    def __init__(
        self,
        llm: BaseChatModel,
        prompt: str = LOGGING_CHECKER_PROMPT,
        parser: BaseOutputParser = JsonOutputParser(
            pydantic_object=ListLoggingCheckerOutput
        ),
    ):
    ...
    def invoke(self, script_content: str):
        ...
        llm_answer = self._chain.invoke(
            {
                "script_content": with_lines,
                "format_instructions": ListLoggingCheckerOutput.model_json_schema(),
            }
        )
